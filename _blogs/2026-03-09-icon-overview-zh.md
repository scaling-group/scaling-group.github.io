...
我们将从这样的数值上下文中获取并应用知识的能力称为[“数值智能”（*numerical intelligence*）]({{ '/numerical-intelligence/' | relative_url }})，并把它视为与语言智能并列的人工智能基础支柱。数值智能直接将测量本身作为上下文：模型感知原始数值观测，从中推断组织这些观测的机制或预测关系，并在无需先将观测翻译成语言的情况下，对陌生系统进行推理。
...
![ICON 选取的一维测试案例]({{ '/images/papers/icon-selected-1d-problems.png' | relative_url }}){: .figure-light-canvas style="width: 100%; display: block; margin: 0 auto"}
